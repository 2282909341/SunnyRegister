...
_log_callback: ContextVar[Any] = ContextVar("kakao_log_callback", default=None)
...
def set_log_callback(callback: Any):
    """Route this invocation's diagnostics into the host worker."""
    return _log_callback.set(callback)


def reset_log_callback(token: Any) -> None:
...
def preflight_proxy(proxy: str, role: str, expected_country: str | None = None) -> tuple[bool, str]:
    ...
    try:
        ...
        status = int(getattr(response, "status_code", 599))
        if status >= 500:
            return False, f"ChatGPT 可达性检测 HTTP {status}"
    except Exception as exc:
        return False, f"ChatGPT 不可达: {str(exc)[:140]}"
    return True, f"{country}，ChatGPT 可达"
...
def kakao_link(
    ...
    *,
    ...
    if me.status_code != 200:
        me_error = response_error(me, 500)
        if me.status_code == 403 and "<html" in me_error.lower():
            log("ChatGPT /me 返回静态 403 页面，跳过非必要预校验并继续创建 Checkout", "[WARN] ")
        else:
